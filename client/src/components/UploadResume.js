...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Download button — sits next to the results title */
  .btn-download {
    display: inline-flex; align-items: center; gap: 7px;
    padding: 9px 16px;
  ...
  .score-right { flex: 1; }
  .score-grade { font-size: 17px; font-weight: 700; margin-bottom: 3px; }
  ...
  /* ── KEYWORD FILTER ── */
  .kw-filter { display: flex; gap: 6px; margin-bottom: 14px; flex-wrap: wrap; }
  ...
  .tag.matched { background: var(--green-bg); border: 1px solid var(--green-bd); color: var(--green); }
  .tag.missing { background: var(--red-bg);   border: 1px solid var(--red-bd);   color: var(--red);   }
  ...
  .cat-name { font-size: 13px; font-weight: 500; color: var(--text); }
  .cat-pct  { font-family: var(--font-mono); font-size: 12px; color: var(--muted); }
  ...
  .priority-badge.high   { background: var(--red-bg);    border: 1px solid var(--red-bd);    color: var(--red);    }
  .priority-badge.medium { background: var(--yellow-bg); border: 1px solid var(--yellow-bd); color: var(--yellow); }
  .priority-badge.low    { background: var(--accent-bg); border: 1px solid rgba(42,92,255,0.2); color: var(--accent); }
  ...
  .empty-results-icon   { font-size: 40px; margin-bottom: 14px; }
  .empty-results-title  { font-family: var(--font-serif); font-size: 20px; color: var(--text); margin-bottom: 8px; }
  .empty-results-sub    { font-size: 13.5px; color: var(--muted); font-weight: 300; line-height: 1.6; }

  .steps { display: flex; flex-direction: column; gap: 12px; margin-top: 24px; text-align: left; }
  ...
  .toast.success { background: #fff; border: 1px solid var(--green-bd); color: var(--green); }
  .toast.error   { background: #fff; border: 1px solid var(--red-bd);   color: var(--red);   }
  ...
  return (
    <div className="score-ring-wrap">
      <svg width="90" height="90" viewBox="0 0 90 90">
        ...
        <circle
          cx="45" cy="45" r={R} fill="none"
          stroke={color.stroke} strokeWidth="5.5" strokeLinecap="round"
          strokeDasharray={circ} strokeDashoffset={offset}
          style={{ transition: "stroke-dashoffset 1.6s cubic-bezier(0.4,0,0.2,1)" }}
        />
      </svg>
      <div className="score-ring-num">{score}<span>ATS</span></div>
    </div>
  );
}
...
export default function UploadResume() {
  ...
  const visibleKw = kwFilter === "all" ? allKw : allKw.filter(k => k.type === kwFilter);

  const suggestions = (result?.suggestions ?? []).map((s, i) => {
    if (typeof s === "object") return s;
    return { text: s, priority: i === 0 ? "high" : i < 3 ? "medium" : "low", category: "" };
  });
    ...
    <>
      ...
      <div className="root">
        ...
        <header className="hd">
          ...
          <h1 className="hd-title">Resume <em>Analyzer</em></h1>
          <p className="hd-sub">Match your resume to any job description — instantly.</p>
        </header>
          ...
          <div className="left-col">
            ...
            <div className="card">
              <div className="card-header">
                ...
                <div>
                  <div className="card-header-title">Resume File</div>
                  <div className="card-header-sub">PDF only</div>
                </div>
              </div>
              <div className="card-body">
                <div className="field" style={{ marginBottom: 18 }}>
                  ...
                  >
                    ...
                    <div className="dz-icon">📁</div>
                    <p className="dz-title">Drop your resume here</p>
                    <p className="dz-sub">or click to browse — PDF only</p>
                    {file && (
                      <div className="dz-chosen">
                        <span>✓</span>
                        <span>{file.name}</span>
            ...
            {!result ? (
              <div className="empty-results">
                ...
                <div className="steps">
                  ...
                  <div className="step-item">
                    <div className="step-num">2</div>
                    <div className="step-text"><strong>Upload your PDF resume</strong></div>
                  </div>
                  <div className="step-item">
                    <div className="step-num">3</div>
                    <div className="step-text"><strong>Get your ATS score</strong> — keyword gaps, coverage & action items</div>
                  </div>
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="score-card">
                  ...
                  <div className="score-card-top-row">
                    <span className="score-card-label">Analysis Report</span>
                    <button
                      className="btn-download"
                      onClick={() => downloadReport(result)}
                      title="Download full report as .txt"
                    >
                      ⬇ Download Report
                    </button>
                  </div>
                  ...
                  <div className="mini-stats">
                    <div className="mini-stat">
                      <div className="mini-stat-val" style={{ color: "#16a34a" }}>{result.matchedKeywords?.length ?? 0}</div>
                      <div className="mini-stat-lbl">Matched</div>
                    </div>
                    <div className="mini-stat">
                      <div className="mini-stat-val" style={{ color: "#dc2626" }}>{result.missingKeywords?.length ?? 0}</div>
                      <div className="mini-stat-lbl">Missing</div>
                    </div>
                    <div className="mini-stat">
                      <div className="mini-stat-val" style={{ color: "#2a5cff" }}>{suggestions.length}</div>
                      <div className="mini-stat-lbl">Actions</div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="tab-content">
                    ...
                    {activeTab === "keywords" && (
                      <div>
                        ...
                        <div className="tags">
                          ...
                          {visibleKw.length === 0 && <p className="empty-muted">No keywords in this category.</p>}
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab === "categories" && (
                      <div className="cat-list">
                        ...
                        {categories.length === 0 && <p className="empty-muted">Category data not available.</p>}
                      </div>
                    )}
                    ...
                    {activeTab === "suggestions" && (
                      <div className="sug-list">
                        {suggestions.map((s, i) => (
                          <div className="sug-item" key={i} style={{ animationDelay: `${i * 0.06}s` }}>
                            <span className="sug-num">{String(i + 1).padStart(2, "0")}</span>
                            <div className="sug-body">
                              {s.category && <div className="sug-category">{s.category}</div>}
                              <div className="sug-text">{s.text ?? s}</div>
                            </div>
                            <span className={`priority-badge ${s.priority ?? "low"}`}>{s.priority ?? "low"}</span>
                          </div>
                        ))}
                        {suggestions.length === 0 && <p className="empty-muted">No suggestions available.</p>}
                      </div>
                    )}
